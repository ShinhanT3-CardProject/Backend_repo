...
public interface ThemeRepository1 extends JpaRepository<ThemeEntity, Long> {
	...
	// 전체 테마 조회
	@Query("SELECT new com.shinhan.soloplay.theme.ThemeSearchDTO1(t.themeId, t.themeName, t.themeIsPublic, mc.themeMainCategoryId, mc.themeMainCategoryName, mc.themeBackground) " + 
				"FROM ThemeEntity t " +
				"JOIN t.themeContents tc " +
				"JOIN tc.subCategory sc " + 
				"JOIN sc.mainCategory mc " +
				"WHERE t.themeIsPublic = 1"
			)
	...
	// 나의 테마 상세조회
	ThemeSearchDTO1 findMyThemeDetail(Long themeId);
