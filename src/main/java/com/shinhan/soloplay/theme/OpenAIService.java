...
public class OpenAIService {
    ...
    public ThemeResponseDTO getTheme(BucketListRequestDTO request) {
        String prompt = "다음 버킷리스트 항목들을 분석하여 아래 주어진 테마들 중에서 가장 관련성이 높은 테마를 선택하고, 그 이유를 3문장으로 리스트 형식으로 간략하게 존댓말로 설명해주세요. 테마 추천 결과는 가능한 테마들 중에 선별해서 무조건 선별할 '테마'만 단어 형태로 알려주고, 설명은 전체적인 이유로 답변해주세요. 한국어로 답변해주세요.\n\n" +
